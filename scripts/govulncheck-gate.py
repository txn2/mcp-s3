...
def check(messages, accepted, allow_path):
    """Judge the report against the accepted set.

    Returns (failures, called, advisories): the failure lines (empty when the
    report is acceptable), the set of advisory ids reaching code we call, and
    every advisory in the report keyed by id.
    """
...
def main():
    ...
    if called:
        ...
        print("govulncheck: no vulnerabilities reached code we call")
    return 0
