...
# --quiet is used rather than reading the diff: under `set -o pipefail`, a
# reader closing the pipe early turns git's SIGPIPE into exit 141 and kills
# the script on exactly the large diffs it most needs to handle.
...
# Untracked Go files are computed HERE, before the skip decision, not only in
# Step 1b. Neither `git diff` form above reports an untracked file, so a branch
# whose only change is a brand-new package would satisfy every "nothing changed"
# test and skip the gate entirely — the one shape of new code most likely to
# arrive without tests. The list is reused by Step 1b rather than recomputed.
# `|| true`: under `set -o pipefail` a grep matching nothing exits 1, and having
# no untracked Go files is the normal case, not an error.
UNTRACKED_GO=$({ git ls-files --others --exclude-standard -- '*.go' 2>/dev/null \
    | grep -v '_test\.go$' || true; })

if [ "$MERGE_BASE" = "$(git rev-parse HEAD)" ] && [ -z "$HAS_UNCOMMITTED" ] && [ -z "$HAS_STAGED" ] && [ -z "$UNTRACKED_GO" ]; then
    echo "SKIP: HEAD is the merge base (on $BASE_BRANCH or no new commits), no uncommitted changes, no untracked Go files."
    exit 0
fi
...
#
# This repo carries no path exclusions. Every non-test .go line this branch
# adds is gated. An exclusion here is invisible to a reviewer reading the
# report, so it has to be worth that cost before it goes in.
...
# Step 1b: Untracked .go files count as changed in full.
#
# `git diff` never reports a file git has not been told about, so a brand-new
# package added but not yet staged was invisible to this gate — and a new file
# with no tests is the most common shape of new untested code, which is exactly
# what a patch gate exists to catch. Every line of such a file is new by
# definition, so all of them are added to the changed set.
#
# `git add -N` would make git diff report them, but it mutates the index, which
# changes the working-tree diff and so invalidates the verify sentinel that
# `make verify` is about to write. Enumerating them read-only avoids that.
printf '%s\n' "$UNTRACKED_GO" \
    | while IFS= read -r untracked; do
        [ -n "$untracked" ] || continue
        [ -r "$untracked" ] || continue
        awk -v f="$untracked" '{ print f, FNR }' "$untracked" || true
